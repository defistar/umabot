...
const web3 = new Web3(new Web3.providers.HttpProvider("https://mainnet.infura.io/v3/e16ecf41380f442987d31e736990fed2"));
...
async function calculateBalancerLPRewards(
  ...
  console.log(`poolAddress : ${poolAddress} - tokenName: ${tokenName} - fromBlock: ${fromBlock} - toBlock: ${toBlock}- week: ${week}- tokenName: ${tokenName} - blocksPerSnapshot: ${blocksPerSnapshot} - umaPerWeek: ${umaPerWeek}`);
  ...
  if (!web3.utils.isAddress(poolAddress) ) {
    throw new Error("Missing or invalid parameter! Provide poolAddress");
  }
  ...
  console.log(`toBlock - fromBlock is: ${(toBlock - fromBlock)} : blocksPerSnapshot: ${blocksPerSnapshot} :   without Ceil: ${((toBlock - fromBlock) / blocksPerSnapshot)} - withCeil div snapshotsToTake: ${snapshotsToTake}`);
  ...
  console.log(`poolInfo: ${Object.keys(poolInfo)}`);
  console.log(`poolInfo: ${JSON.stringify(poolInfo.shares)} - ${poolInfo.shares.length}`);
  ...
  console.log(`bPool keys extracted: ${Object.keys(bPool)}`);
...
async function _updatePayoutAtBlock(bPool, blockNumber, shareHolderPayout, umaPerSnapshot) {
  ...
  // Get the given holders
  const shareHoldersMap = Object.keys(shareHolderPayout).map(result => {return result});
  console.log("\n");
  for(i=0;i < balanceResults.length;i++){
    console.log(`Shareholder: ${shareHoldersMap[i]} has balance: ${balanceResults[i]} @Block#: ${blockNumber}`);
  }  
  console.log("\n");

  // For each balance result, calculate their associated payment addition.
  balanceResults.forEach(function(balanceResult, index) {
    console.log(`Shareholder: ${shareHoldersMap[index]} has BPT-balance : ${balanceResults[index]}`);
...
async function _fetchBalancerPoolInfo(poolAddress) {
  ...
  console.log(`response for subgraph request ${SUBGRAPH_URL} is: ${JSON.stringify(response)}`);

  const data = (await response.json()).data;
  if (data.pools.length > 0) {
    console.log(`response for subgraph request ${SUBGRAPH_URL} datapools is: ${JSON.stringify(data.pools[0])}`);
    return data.pools[0];
  }
